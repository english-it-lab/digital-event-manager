...
class Faculty(Base):
    ...
    id: Mapped[int] = mapped_column(primary_key=True)
    university_id: Mapped[Optional[int]] = mapped_column(ForeignKey("universities.id", ondelete="SET NULL"))
    name: Mapped[str] = mapped_column(String(255), nullable=False)

    university: Mapped[Optional[University]] = relationship(back_populates="faculties")
...
class Person(Base):
    ...
    workplace: Mapped[Optional[str]] = mapped_column(String(50))
    tg_name: Mapped[Optional[str]] = mapped_column(String(32))
...
class Organizer(Base):
    ...
    access_key: Mapped[Optional[int]] = mapped_column(BigInteger)

    person: Mapped[Optional[Person]] = relationship(back_populates="organizers")
...
class Event(Base):
    ...
    organizer_id: Mapped[Optional[int]] = mapped_column(ForeignKey("organizers.id", ondelete="SET NULL"))
    ...
    organizer: Mapped[Optional[Organizer]] = relationship(back_populates="events")
...
class TextbookLevel(Base):
    ...
    id: Mapped[int] = mapped_column(primary_key=True)
    level_abbreviation: Mapped[Optional[str]] = mapped_column(String(3))
...
class Section(Base):
    ...
    lecture_hall: Mapped[Optional[str]] = mapped_column(String(7))
    time: Mapped[Optional[DateTime]] = mapped_column(DateTime(timezone=True))
...
class Topic(Base):
    ...
    id: Mapped[int] = mapped_column(primary_key=True)
    section_id: Mapped[Optional[int]] = mapped_column(ForeignKey("sections.id", ondelete="SET NULL"))
    name: Mapped[Optional[str]] = mapped_column(String(50))

    section: Mapped[Optional[Section]] = relationship(back_populates="topics")
...
class Group(Base):
    ...
    registration_time: Mapped[Optional[DateTime]] = mapped_column(DateTime(timezone=True))

    section: Mapped[Optional[Section]] = relationship(back_populates="groups")
...
class GroupTopic(Base):
    ...
    topic: Mapped[Optional[Topic]] = relationship(back_populates="group_topics")
...
class Teacher(Base):
    ...
    university: Mapped[Optional[University]] = relationship(back_populates="teachers")
    department: Mapped[Optional[Department]] = relationship(back_populates="teachers")
    person: Mapped[Optional[Person]] = relationship(back_populates="teachers")
...
class Participant(Base):
    ...
    textbook_level_id: Mapped[Optional[int]] = mapped_column(ForeignKey("textbook_levels.id", ondelete="SET NULL"))
    is_group_leader: Mapped[bool] = mapped_column(Boolean, default=False, server_default="false")
    presentation_topic: Mapped[Optional[str]] = mapped_column(String(255))
    is_notification_allowed: Mapped[bool] = mapped_column(Boolean, default=True, server_default="true")
    password_hash: Mapped[Optional[str]] = mapped_column(String(255))

    person: Mapped[Optional[Person]] = relationship(back_populates="participants")
    faculty: Mapped[Optional[Faculty]] = relationship(back_populates="participants")
    course: Mapped[Optional[Course]] = relationship(back_populates="participants")
    teacher: Mapped[Optional[Teacher]] = relationship(back_populates="participants")
    section: Mapped[Optional[Section]] = relationship(back_populates="participants")
    textbook_level: Mapped[Optional[TextbookLevel]] = relationship(back_populates="participants")
...
class GroupParticipant(Base):
    ...
    __table_args__ = (
        UniqueConstraint("group_id", "participant_id", name="uq_group_participant"),
    )
...
class Jury(Base):
    ...
    university: Mapped[Optional[University]] = relationship(back_populates="juries")
    person: Mapped[Optional[Person]] = relationship(back_populates="juries")
...
class SectionJury(Base):
    ...
    __table_args__ = (
        UniqueConstraint("section_id", "jury_id", name="uq_section_jury"),
    )
...
class JuryScore(Base):
    ...
    id: Mapped[int] = mapped_column(primary_key=True)
    jury_id: Mapped[Optional[int]] = mapped_column(ForeignKey("juries.id", ondelete="SET NULL"))
    participant_id: Mapped[Optional[int]] = mapped_column(ForeignKey("participants.id", ondelete="SET NULL"))
    organization_score: Mapped[Optional[float]] = mapped_column(Float)
    content: Mapped[Optional[float]] = mapped_column(Float)
    visuals: Mapped[Optional[float]] = mapped_column(Float)
    mechanics: Mapped[Optional[float]] = mapped_column(Float)
    delivery: Mapped[Optional[float]] = mapped_column(Float)
    comment: Mapped[Optional[str]] = mapped_column(String(100))

    jury: Mapped[Optional[Jury]] = relationship(back_populates="jury_scores")
    participant: Mapped[Optional[Participant]] = relationship(back_populates="jury_scores")
...
class JuryScoreChange(Base):
    ...
    jury_id: Mapped[Optional[int]] = mapped_column(ForeignKey("juries.id", ondelete="SET NULL"))
    update_time: Mapped[Optional[DateTime]] = mapped_column(DateTime(timezone=True), server_default=func.now())

    jury_score: Mapped[JuryScore] = relationship(back_populates="changes")
    jury: Mapped[Optional[Jury]] = relationship(back_populates="jury_score_changes")
...
class OrganizerSectionChange(Base):
    ...
    organizer: Mapped[Optional[Organizer]] = relationship(back_populates="section_changes")
...
class OrganizerParticipantChange(Base):
    ...
    organizer: Mapped[Optional[Organizer]] = relationship(back_populates="participant_changes")
...
class TechnicalRequirement(Base):
    ...
    sizes: Mapped[Optional[str]] = mapped_column(String(2))

    topic: Mapped[Optional[Topic]] = relationship(back_populates="technical_requirements")
...
class PosterContent(Base):
    ...
    id: Mapped[int] = mapped_column(primary_key=True)
    technical_requirements_id: Mapped[Optional[int]] = mapped_column(ForeignKey("technical_requirements.id", ondelete="SET NULL"))
    words_amount: Mapped[Optional[int]] = mapped_column(Integer)
    images_amount: Mapped[Optional[int]] = mapped_column(Integer)

    technical_requirement: Mapped[Optional[TechnicalRequirement]] = relationship(back_populates="posters_content")
