...
class TestNotificationsAPI:
    ...
    def test_tc01_send_notifications_success(self, client):
        ...
        Мокаем NotificationService через правильный путь
        """
        # Мокаем сервис ТАМ, ГДЕ ОН ИСПОЛЬЗУЕТСЯ - в адаптере
        with patch("app.adapters.api.v1.notifications.NotificationService") as MockService:
            ...
            # Проверяем, что эндпоинт вызвал наш мок
            mock_instance.send_draw_notifications.assert_called_once()
    ...
    def test_tc03_event_not_found(self, client):
        with patch("app.adapters.api.v1.notifications.NotificationService") as MockService:
            ...
            # В зависимости от обработки ошибки в адаптере
    ...
    def test_tc11_database_error(self, client):
        with patch("app.adapters.api.v1.notifications.NotificationService") as MockService:
            mock_instance = AsyncMock()
            MockService.return_value = mock_instance
            mock_instance.send_draw_notifications = AsyncMock(side_effect=Exception("DB error"))
    ...
    def test_tc12_smtp_error(self, client):
        with patch("app.adapters.api.v1.notifications.NotificationService") as MockService:
            mock_instance = AsyncMock()
            MockService.return_value = mock_instance
            mock_instance.send_draw_notifications = AsyncMock(side_effect=Exception("SMTP error"))
